...
- **Email signups:** replace `https://formspree.io/f/YOUR_ID` in `FORM_ENDPOINT` with a Formspree form URL or a compatible endpoint that accepts `POST` JSON shaped as `{ "email": "..." }`. While the placeholder is present, valid submissions show the complete success state without sending or persisting data; a refresh resets the demo.
...
- [x] Styled email-provider autocomplete with keyboard support, strict format/typo checks, inline feedback, loading/success states, and honeypot
...
- [x] Best-effort audio autoplay with first-interaction fallback for mobile browser restrictions
